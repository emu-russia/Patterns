...
void SaveWorkspace(char *filename)
{
	...
	ws.RowArrangement = WorkspaceRowArrangement;
	ws.Flag = 0;
	...
	if (ImageName)
	{
		...
		ws.SourceImageOffset = sizeof(WorkspaceImage);
	...
	if (ws.PatternsAdded) ws.PatternsLayerOffset = sizeof(WorkspaceImage) + ws.SourceImageLength;
...
void LoadWorkspace(char *filename)
{
	...
	WorkspaceRowArrangement = ws.RowArrangement;
